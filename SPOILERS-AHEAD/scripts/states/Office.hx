// lwk just kidnapped this shit from the legacy source code X3
...
import openfl.display.PNGEncoderOptions;
	...
	var enteredCode:String = "HELLSCAAAAAAAAAA";
	...
	function onCreate()
	{
        FlxG.save.data.initiativeHintCode = '';
        FlxG.save.data.didthething = false;
        //FlxG.save.data.officeStateVisited = false;
		
		if (FlxG.save.data.initiativeHintCode == '')
		{
			var hintCodes = [
				'FEAR', 'PORT'
			];
			var hintWeights = [for (code in hintCodes) getHintCodeWeight(code)];
			FlxG.save.data.initiativeHintCode = randomFromArrayWeighted(hintCodes, hintWeights);
			ClientPrefs.flush();
		}
		...
		dialogueLines = ['OH THANK GOD, I\'M OUT OF THERE.', '...The hell is this room?'];
		postDialogueLines = ['This looks like a security office... similar to mine...', 'I should probably check around.'];
		...
		if (FlxG.save.data.officeStateVisited)
		{
		...
		{
			FlxG.save.data.officeStateVisited = true;
			dialogueActive = true;
	...
	function openComputer()
	{
		...
		applyCameraFilters();
		passwordReady = false;
		if (FlxG.save.data.didthething) enteredCode = "";
		glowHovering = false;
		...
		FlxTween.tween(FlxG.camera, {zoom: 4.1}, 0.45,
			{
				...
				onComplete: function(_) {
                    ...
                    FlxG.save.data.didthething = true;
				}
			});
	}
	...
	function buildComputerUI()
	{
		...
		var hintStr = FlxG.save.data.initiativeHintCode;
			...
			FlxTween.tween(fade, {alpha: 1}, 0.5,
				{
					...
					onComplete: function(_) {
						...
						FlxG.switchState(()->{
                            new ScriptedState("initiative/dave/MainMenu");
                        });
					}
				});
			return;
	...
	function getHintCodeWeight(code:String):Float
	{
		return switch (code)
		{
			case 'GAMEBOYD': 1;
	...
	function randomFromArrayWeighted(v:Array<Dynamic>, weights:Array<Float>):Dynamic
	{
